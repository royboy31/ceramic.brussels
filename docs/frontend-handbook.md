...
| VIP: about `897:1285`, programme `963:400`, lounge `982:776`, hotel deal `998:1083`, access (locked state) `1026:1895` | `src/components/hubs/Vip.astro` via `[hub]/[...tab].astro`; the forms are `VipCodeForm.astro` and `VipRequestForm.astro` | `getHubPages('vip')`, `getProgramme` (section `vip`, `venue`), `getVipSettings` | `page`, `programmeEvent`; the gate and the code store are Kamindu's (`docs/vip-access.md`) |
| About: the fair `289:671`, advisory board `289:767`, contact `713:445` | `src/components/hubs/About.astro` via `[hub]/[...tab].astro` | `getHubPages('about')`, `getPeople`, `getSettings` | `page`, `person`, `siteSettings` (contact block) |
| Press & media: stories `1108:725`, press `1108:887`, photos & videos and media partners (hand-off of 2026-09-21) | `src/components/hubs/PressMedia.astro` (layout) and `src/components/PressMedia.astro` (the views) via `[hub]/[...tab].astro` | `getHubPages('press-media')`, `getStories`, `getPressClips`, `getNews` (press releases), `getEditions` (gallery, film, press reviews), `getPartners` (`media` tier), `getSettings` (agencies, press kit, collectors' voices link) | `page` (`intro`, `intro2`), `story`, `pressClip`, `newsItem`, `edition`, `partner`, `siteSettings` |
