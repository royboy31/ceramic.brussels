...
const BLOCKS = {
  ...
  spacerSection: () => `${text(M, 26, CW, 3)}<rect x="${M}" y="72" width="${CW}" height="56" fill="none" stroke="${MUTED}" stroke-width="2" stroke-dasharray="6 4"/><line x1="${W / 2}" y1="80" x2="${W / 2}" y2="120" stroke="${INK}" stroke-width="2"/><polygon points="${W / 2 - 5},86 ${W / 2 + 5},86 ${W / 2},78" fill="${INK}"/><polygon points="${W / 2 - 5},114 ${W / 2 + 5},114 ${W / 2},122" fill="${INK}"/>${text(M, 142, CW, 3)}`,
  ...
  partnersSection: () => `${line(M, 34, 100, 8)}${rule(M, 48, CW)}${[0, 1, 2, 3].map((i) => `${rect(M + i * 70, 76, 56, 40, MUTED, 'rx="4"')}${line(M + i * 70 + 8, 90, 40, 12, '#b0b0b0')}`).join('')}${[0, 1, 2, 3].map((i) => `${rect(M + i * 70, 132, 56, 40, MUTED, 'rx="4"')}${line(M + i * 70 + 8, 146, 40, 12, '#b0b0b0')}`).join('')}`,
  ...
  applicationFormSection: () => `${[0, 1].map((r) => [0, 1].map((c) => `${line(M + c * 144, 44 + r * 52, 60, 6, MUTED)}${rule(M + c * 144, 70 + r * 52, 128)}`).join('')).join('')}${pill(M, 158, 70, INK)}`,
