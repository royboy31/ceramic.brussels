...
| `page` | hub tabs, `/[lang]/[...slug]` | With a `section` it becomes a pill tab of that hub (about, press-media, art-prize, programme, vip, visit, partners); without one it is a standalone page with **slug per locale**. A lead paragraph, a cover, then a **section stack** the editor composes from pre-designed blocks (see `pageTemplate`), and closing images. |
...
| `pressClip` | press & media → press | "As seen in the press": cover, date, title, outlet, link. |
| `story` | press & media → stories | An interview or a collectors' voice: a card with a picture, a few lines and a link to where the piece lives (a news item, a page, Ceramics Now). |
